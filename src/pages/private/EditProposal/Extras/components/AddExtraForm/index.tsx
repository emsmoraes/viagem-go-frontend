...
import { CreateExtraRequest } from "@/shared/services/entities";
import { useSteppers } from "../../../contexts/SteppersContext/useSteppers";
import { useCreateExtraMutation } from "../../hooks/useExtra";
import MoneyInput from "@/shared/components/Form/MoneyInput";

const addExtraFormSchema = z.object({
...
export type ExtraSchema = z.infer<typeof addExtraFormSchema>;
...
function AddExtraForm({ setOpen }: { setOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  const queryClient = useQueryClient();
  const { proposal } = useSteppers();

  const { createExtra, isLoadingCreateExtra } = useCreateExtraMutation({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["proposal", proposal?.id] });
      form.reset();
      setOpen(false);
      toast("Extra criado com sucesso!");
    },
    onError: () => {
      toast("Erro ao criar extra.");
    },
  });

  const form = useForm<ExtraSchema>({
    resolver: zodResolver(addExtraFormSchema),
    defaultValues: {
      title: "",
      description: "",
      price: 0,
  ...
  const onSubmit = (values: ExtraSchema) => {
    if (!proposal) return;

    const payload: CreateExtraRequest = {
      ...values,
      images,
      files,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Tipo de extra" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Descrição do extra" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <Button type="submit" disabled={isLoadingCreateExtra} className="flex-1 [&_svg]:size-6">
          {isLoadingCreateExtra ? <CgSpinner className="animate-spin" /> : "Salvar"}
        </Button>
      </form>
    </Form>
  );
