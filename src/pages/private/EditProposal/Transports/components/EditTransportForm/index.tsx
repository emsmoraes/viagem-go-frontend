...
import { UpdateTransportRequest } from "@/shared/services/entities";
import { useUpdateTransportMutation } from "../../hooks/useTransport";
...
export type EditTransportSchema = z.infer<typeof transportSchema>;
...
type EditTransportFormProps = {
  defaultValues: EditTransportSchema;
  transportId: string;
};

function EditTransportForm({ defaultValues, transportId }: EditTransportFormProps) {
  const { proposal } = useSteppers();
  const queryClient = useQueryClient();

  const { updateTransport, isLoadingUpdateTransport } = useUpdateTransportMutation({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["proposal", proposal?.id] });
      form.reset(form.watch());
      toast("Transporte atualizado com sucesso!");
    },
    onError: () => {
      toast("Erro ao atualizar transporte");
    },
  });

  const form = useForm<EditTransportSchema>({
  ...
  const onSubmit = (values: EditTransportSchema) => {
    if (!proposal || !transportId) return;
    ...
    const formData: UpdateTransportRequest = {
      ...values,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Tipo do transporte" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Descrição detalhada do transporte" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <Button
          ...
          disabled={!isFormDirty || isLoadingUpdateTransport}
        >
          {isLoadingUpdateTransport ? <CgSpinner className="animate-spin" /> : "Atualizar"}
        </Button>
      </form>
    </Form>
  );
