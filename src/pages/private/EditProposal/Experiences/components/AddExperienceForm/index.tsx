...
import { CreateExperienceRequest } from "@/shared/services/entities";
import { useSteppers } from "../../../contexts/SteppersContext/useSteppers";
import { useCreateExperienceMutation } from "../../hooks/useExperience";
...
export type ExperienceSchema = z.infer<typeof addExperienceFormSchema>;
...
function AddExperienceForm({ setOpen }: { setOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  const queryClient = useQueryClient();
  const { proposal } = useSteppers();

  const { createExperience, isLoadingCreateExperience } = useCreateExperienceMutation({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["proposal", proposal?.id] });
      form.reset();
      setOpen(false);
      toast("Experiência criada com sucesso!");
    },
    onError: () => {
      toast("Erro ao criar experiência.");
    },
  });
  ...
  const onSubmit = (values: ExperienceSchema) => {
    if (!proposal) return;
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Tipo de experiência" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Descrição do experiência" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <Button type="submit" disabled={isLoadingCreateExperience} className="flex-1 [&_svg]:size-6">
          {isLoadingCreateExperience ? <CgSpinner className="animate-spin" /> : "Salvar"}
        </Button>
      </form>
    </Form>
  );
